...
Public Class Goa_Package

    Dim pkvar As String
    Sub saveRecord()
        If conn.State = ConnectionState.Open Then conn.Close()
        conn.Open()
        q1var = "Insert into Goa("
        q2var = "values("
    ...
    Sub disrecords()
        ...
        Dim adp As New SqlDataAdapter("select * from Goa", conn)
        adp.Fill(DS1)
    ...
    Private Sub Butdelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Butdelete.Click
        ...
        If conn.State = ConnectionState.Open Then conn.Close()
        conn.Open()
        Dim cmd1 As New SqlCommand("Delete from Goa where Packageid =" & Val(TextBox1.Text) & "", conn)
        cmd1.ExecuteNonQuery()
        If conn.State = ConnectionState.Open Then conn.Close()
        disrecords()
        TextBox1.Text = ""
    ...
    Private Sub DG1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG1.CellContentClick
        ...
        If conn.State = ConnectionState.Open Then conn.Close()
        conn.Open()
        Dim cmd0 As New SqlCommand("select Packageid,Packagename,Days,Nights,Foodtype,Accomodation,Foodcharge,Accomomodationcharge,Travellingcharge,Miscellaneous,Total,Places from Goa where Packageid ='" & pkvar & "'", conn)
    ...
    Private Sub Goa_Package_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        disrecords()
    End Sub
