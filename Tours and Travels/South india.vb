...
Public Class South_india
    ...
    Sub disrecords()
        ...
        Dim adp As New SqlDataAdapter("select * from Southindia", conn)
        adp.Fill(DS1)
    ...
    Private Sub DG1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
        ...
        If conn.State = ConnectionState.Open Then conn.Close()
        conn.Open()
        Dim cmd0 As New SqlCommand("select Packageid,Packagename,Days,Nights,Foodtype,Accomodation,Foodcharge,Accomodationcharge,Travellingcharge,Miscellaneous,Total,Places from Southindia where Packageid ='" & pkvar & "'", conn)
    ...
    Private Sub South_india_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        disrecords()
    End Sub
    ...
    Private Sub DG1_CellContentClick_1(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG1.CellContentClick
        ...
        If conn.State = ConnectionState.Open Then conn.Close()
        conn.Open()
        Dim cmd0 As New SqlCommand("select Packageid,Packagename,Days,Nights,Foodtype,Accomodation,Foodcharge,Accomomodationcharge,Travellingcharge,Miscellaneous,Total,Places from Southindia where Packageid ='" & pkvar & "'", conn)
        Dim d1 As SqlDataReader = cmd0.ExecuteReader()
        If d1.HasRows Then
            d1.Read()
            TextBox1.Text = d1(0).ToString
