...
Public Class North_india
    ...
    Sub disrecords()
        ...
        Dim adp As New SqlDataAdapter("select * from Northindia", conn)
        adp.Fill(DS1)
    ...
    Private Sub Butmodify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Butmodify.Click
        ...
        If conn.State = ConnectionState.Open Then conn.Close()
        conn.Open()
        Dim cmd1 As New SqlCommand("Delete from Northindia where Packageid =" & Val(TextBox1.Text) & "", conn)
        cmd1.ExecuteNonQuery()
        If conn.State = ConnectionState.Open Then conn.Close()
        saveRecord()
    End Sub
    ...
    Private Sub DG1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG1.CellContentClick
        ...
        If conn.State = ConnectionState.Open Then conn.Close()
        conn.Open()
        Dim cmd0 As New SqlCommand("select Packageid,Packagename,Days,Nights,Foodtype,Accomodation,Foodcharge,Accomomodationcharge,Travellingcharge,Miscellaneous,Total,Places from Northindia where Packageid ='" & pkvar & "'", conn)
    ...
    Private Sub North_india_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        disrecords()
    End Sub
